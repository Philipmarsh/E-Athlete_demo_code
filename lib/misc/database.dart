...
//     for (Session session in sessions){
//       await dbClient.transaction((txn) async{
//         return await txn.rawUpdate('UPDATE Sessions' +
...
//     for (Result result in results){
//       await dbClient.transaction((txn) async{
//         return await txn.rawUpdate('UPDATE Results' +
...
//     var dbClient = await db;
//     List<Map> list = await dbClient.rawQuery('SELECT * FROM Goals WHERE goalType = \'Ultimate\'');
//     List<Goal> goalList = [];
//     for (var goal in list) {
//       Goal newGoal = Goal();
//       newGoal.id = goal['id'];
...
//     var dbClient = await db;
//     List<Map> list = await dbClient.rawQuery('SELECT * FROM Goals WHERE goalType = \'Finished\'');
//     print(list);
//     List<Goal> goalList = [];
//     for (var goal in list) {
//       Goal newGoal = Goal();
...
//             ") VALUES(" +
//             "?, ?, ?, ?, ?, ?)",
//             [goal.id, goal.deadlineDate, goal.setOnDate, goal.content, goal.goalType, goal.achieved]);
//       });
//     }
...
//             ' WHERE id = ?',
//             [goal.deadlineDate, goal.setOnDate, goal.content, goal.goalType, goal.achieved, goal.id]);
//       });
//     }
//     print('goal updated');
//   }
...
//       await dbClient.transaction((txn) async {
//         return await txn.rawInsert('INSERT INTO  Objectives' +
//             "(id, endDate, hoursOfWork, average, startDate, objectiveType, isFinished" +
//             ") VALUES(" +
//             "?, ?, ?, ?, ?, ?, ?)",
//             [objective.id, objective.endDate.toIso8601String(), objective.hoursOfWork, objective.average, objective.startDate.toIso8601String(),
//               objective.objectiveType, objective.isFinished]);
//       });
//     }
...
//     for (Objective objective in objectives){
//       await dbClient.transaction((txn) async {
//         return await txn.rawDelete('DELETE FROM Objectives WHERE id = ?',
//             [objective.id]);
//       });
//     }
//   }
...
//     for (Objective objective in objectives){
//       await dbClient.transaction((txn) async{
//         return await txn.rawUpdate('UPDATE Objectives' +
//             ' SET endDate = ?, hoursOfWork = ?, average = ?, startDate = ?, objectiveType = ?,' +
//             ' isFinished = ? WHERE id = ?',
//             [ objective.endDate.toIso8601String(), objective.hoursOfWork, objective.average, objective.startDate.toIso8601String(),
//               objective.objectiveType, objective.isFinished, objective.id]);
//       });
//     }
